...
ggplot(possar[[1]], aes(x=diff, y=win)) +
  ...
  theme_bw() +
  theme(panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text=element_text(size=13), axis.title=element_text(size=16), plot.title =  element_text(size = 20, face = "bold"))
...
ggplot(possar[[2]], aes(x=diff, y=win)) +
  geom_point() + stat_smooth(method="glm", method.args = list(family = binomial(link = "logit")), se=FALSE) +
  ggtitle(paste0("Logistisk regression för ", pos_list[2])) +
  labs(x="MMR-differens mot laget", y="Vinstsannolikhet")  +
  theme_bw() +
  theme(panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text=element_text(size=13), axis.title=element_text(size=16), plot.title =  element_text(size = 20, face = "bold"))
...
ggplot(possar[[3]], aes(x=diff, y=win)) +
  geom_point() + stat_smooth(method="glm", method.args = list(family = binomial(link = "logit")), se=FALSE) +
  ggtitle(paste0("Logistisk regression för ", pos_list[3])) +
  labs(x="MMR-differens mot laget", y="Vinstsannolikhet")  +
  theme_bw() +
  theme(panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text=element_text(size=13), axis.title=element_text(size=16), plot.title =  element_text(size = 20, face = "bold"))
...
ggplot(possar[[4]], aes(x=diff, y=win)) +
  geom_point() + stat_smooth(method="glm", method.args = list(family = binomial(link = "logit")), se=FALSE) +
  ggtitle(paste0("Logistisk regression för ", pos_list[4])) +
  labs(x="MMR-differens mot laget", y="Vinstsannolikhet")  +
  theme_bw() +
  theme(panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text=element_text(size=13), axis.title=element_text(size=16), plot.title =  element_text(size = 20, face = "bold"))
...
```{r, echo=FALSE,fig.width=6.4,fig.height=6.4}
  ...
  with(ratios, {# this allows entering your commands more succinctly
    ...
    plot(1:nr, boxOdds, ylim = c(0.9990, 1.001), type = 'n', xaxt = 'n', xlab = '', main="Oddskvot och 95% CL för de förklarande variablerna", cex = 25, ylab = 'Oddskvot', frame.plot = TRUE, panel.first = grid(nx = NA, ny = NULL))
...
plot(prf,col="blue",lwd=2,main="ROC kurva för hardlanespelare", cex = 25, ylab = "Andel sant positiv", xlab= "Andel Falsk positiv")
#title(main = "ROC kurva för hardlanespelare", xlab="", ylab="")
abline(0,1)
 legend(0.45,0.45,c(maxauct),border="white",cex=1.3, pt.cex=1,box.col = "white")
```
